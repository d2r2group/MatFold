...
class MatFold:
    ...
    def __init__(self, df: pd.DataFrame, bulk_df: dict,
                 ...
                 verbose: bool = False, cols_to_keep: list | None = None) -> None:
        """
        ...
        space_groups = dict(
            [(id_, SpacegroupAnalyzer(structures[id_], 0.1).get_space_group_symbol())
        ...
        df_split['sgnum'] = [str(space_groups[id_]) for id_ in df_split['structure_id']]
    ...
    def create_splits(self, split_type: str, n_inner_splits: int = 10, n_outer_splits: int = 10,
                      keep_binaries_in_train: bool = True,
                      write_base_str: str = 'mf', output_dir: str | os.PathLike | None = None) -> None:
        """
        Creates splits based on split_type.
        :param split_type: Defines the type of splitting, must be either "index", "structure_id",
        "composition", "chemsys", "sgnum", or "element"
        ...
        if split_type not in ["index", "structure_id", "composition", "chemsys", "sgnum", "element"]:
            raise ValueError('Error: `split_type` must be either "index", "structure_id", '
                             '"composition", "chemsys", "sgnum", or "element"')
        if split_type == "element":
            self.create_splits_by_elements(n_inner_splits=n_inner_splits,
        ...
        if keep_binaries_in_train:
            ...
            test_possibilities = np.array(list(set(out_df[~out_df['isbinary']][split_type])))
        else:
            default_train_indices = []
            test_possibilities = np.array(list(set(out_df[split_type])))
        ...
        for i, (train_outer_set_index, test_outer_set_index) in enumerate(kf_outer.split(test_possibilities)):
            ...
            self.check_split(out_df, [outer_train_df, outer_test_df], verbose=self.verbose)
            ...
            for j, (train_inner_index_index, test_inner_index_index) in (
                ...
                self.check_split(out_df, [outer_test_df, inner_train_df, inner_test_df], verbose=self.verbose)

    def create_splits_by_elements(self, n_inner_splits: int = 10, keep_binaries_in_train: bool = True,
                                  write_base_str: str = 'mf', output_dir: str | os.PathLike | None = None) -> None:
        """
        Creates splits by elements
        :param n_inner_splits: Number of inner splits (nested k-fold)
        :param keep_binaries_in_train: Whether to always keep binaries in training set
        :param write_base_str: Beginning string of csv file names of the written splits
        :param output_dir: Directory where the splits are written to
        :return: None
        ...
        if keep_binaries_in_train:
            # default_train_indices = list(out_df[out_df['n_elements'] == 2].index)
            default_train_indices = list(out_df[out_df['isbinary']].index)
            test_possibilities = np.array(list(set(
                itertools.chain.from_iterable(out_df[~out_df['isbinary']]['elements'])
            )))
        else:
            default_train_indices = []
            test_possibilities = np.array(list(set(
                itertools.chain.from_iterable(out_df['elements'])
            )))

        if self.verbose:
            print('Possible test examples: ', set(test_possibilities))
        kf_outer = KFold(n_splits=len(test_possibilities), random_state=self.return_seed, shuffle=True)
        kf_inner = KFold(n_splits=n_inner_splits, random_state=self.return_seed, shuffle=True)
...
if __name__ == "__main__":
    ...
    mf.create_splits("sgnum", output_dir='./output/')
