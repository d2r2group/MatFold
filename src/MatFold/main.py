...
class MatFold:
    ...
    def __init__(
        ...
        self.df["composition"] = [
            structures[id_].composition.reduced_formula
            for id_ in self.df["structureid"]
        ]

        self.df["chemsys"] = [
            structures[id_].composition.chemical_system
            for id_ in self.df["structureid"]
        ]
        ...
        self.df["elements"] = [
            structures[id_].composition.get_el_amt_dict().keys()
            for id_ in self.df["structureid"]
        ]
    ...
    def from_json(
        ...
        Previously generated by the `create_splits` or `create_loo_split` methods. The same
        `df` and `bulk_dict` used during the original split creation must be provided to
        guarantee that the exact splits are regenerated.
    ...
    def split_statistics(self, split_type: str) -> dict:
        """Analyze statistics of material properties.
        ...
        if split_type in ["elements", "periodictablerows", "periodictablegroups"]:
            ...
            statistics = dict.fromkeys(list(set(self.df[split_type])), 0.0)
    ...
    def create_nested_splits(
        ...
        split_possibilities = self._get_unique_split_possibilities(
            keep_n_elements_in_train,
            split_type,
        )
        ...
        for i, (
            ...
            outer_train_indices, outer_test_indices = self._get_split_indices(
                outer_train_set,
                outer_test_set,
                default_train_indices,
                split_type,
            ...
            if not self._check_split_indices_passed(
            ...
            outer_train_df, outer_test_df = self._save_split_dfs(
                outer_train_indices,
                outer_test_indices,
                default_train_indices,
                path_outer,
            ...
            if kf_inner is not None and inner_equals_outer_split_strategy:
                ...
                for j, (
                    ...
                    inner_train_indices, inner_test_indices = self._get_split_indices(
                        inner_train_set,
                        inner_test_set,
                        default_train_indices,
                        split_type,
                    ...
                    inner_train_df, inner_test_df = self._save_split_dfs(
                        inner_train_indices,
                        inner_test_indices,
                        default_train_indices,
                        path_inner,
    ...
    def _check_split_dfs(
        ...
        if sum(sizes_list) != len(self.df):
            raise Exception(
                f"Error: Non-equal num of indices in splits {sum(sizes_list)} vs. original {len(self.df)}.",
            )
        ...
        if len(keep_n_elements_in_train) > 0:
            if split_type in ["elements", "periodictablerows", "periodictablegroups"]:
                split_possibilities = sorted(
                    {
                        item
                        for sublist in self.df[
                            ~self.df["nelements"].isin(keep_n_elements_in_train)
                        ][split_type]
                        for item in sublist
                    }
                )
                ...
                split_possibilities = sorted(
                    set(
                        self.df[~self.df["nelements"].isin(keep_n_elements_in_train)][
                            split_type
                        ]
                    )
                )
            ...
            split_possibilities = sorted(
                {item for sublist in self.df[split_type] for item in sublist}
            )
        else:
            split_possibilities = sorted(set(self.df[split_type]))
